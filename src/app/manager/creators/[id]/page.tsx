...
import { markCreatorAsViewedByManagerAction } from "@/app/actions/manager";
import { StatusBadge } from "@/components/StatusBadge";
import { VideoEmbedPlayer } from "@/components/VideoEmbedPlayer";
import { isManagerForCountry } from "@/lib/types";
import { ManagerCreatorReviewConsole } from "@/components/ManagerCreatorReviewConsole";
...
export default async function ManagerCreatorReviewPage({ params }: PageProps) {
  ...
  // Location-based scoping: manager cannot access creators from unassigned countries
  if (user.assignedCountry && !isManagerForCountry(user.assignedCountry, profile.country)) {
    redirect("/manager/dashboard");
  }

  if (profile.status === "PENDING_REVIEW" || !profile.viewedAt) {
    await markCreatorAsViewedByManagerAction(profile.id);
  }

  // Fetch projects matching this creator's country or Global
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6 bg-zinc-50 min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-zinc-200">
        ...
        >
          ...
          Back to {user.assignedCountry} Pipeline
        </Link>

        <div className="flex items-center gap-2.5">
          <span className="text-[11px] font-medium px-2 py-0.5 rounded bg-zinc-100 text-zinc-700 border border-zinc-200 flex items-center gap-1">
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-6 space-y-5 shadow-xs">
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded bg-zinc-100 text-zinc-700 border border-zinc-200">
                  {profile.country}
                </span>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-2.5 rounded-lg bg-zinc-50 border border-zinc-200/80">
            ...
            <span className="text-zinc-900 font-medium block truncate">
              {profile.platforms || "TikTok, Reels"}
            </span>
          </div>
        </div>
